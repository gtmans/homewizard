/* 2DO M5MatrixAutoSocketSwitchv8
 *  docu
 *  github
 *  
 *  Mans Rademaker maart 2024
...
//    M5.dis.drawpix  (hor[12],green);    //green point middle screen
    ...
    for               (int i=0;i<4;i++){  // serial print collected data
        ...
        Serial.printf ("Sock%d = %-*s %-13s %8.2f watt %s\n", i, 12, ipnames[i].c_str(), ipDevice.c_str(), sock_usage[i], sock_status[i].c_str());
    }
    Serial.printf     ("Total = %-*s %-13s %8.2f watt", 12, " ", " ", tot_usage);
    Serial.println    (" van " + String   (max_power)+" watt");
    if (missed)       {Serial.println     ("* dummy 999 watt");Serial.println();}
...
/*
//https://nos.nl/teletekst dummy webpage 2 show access works
void TestHTTPAtom       (){
  Serial.print          (">>>>> TestHTTPAtom()");
  if ((wifiMulti.run    () == WL_CONNECTED)) {                      // wait for WiFi connection.
  Serial.print          ("[HTTP] begin...\n");
  http.begin            ("http://nos.nl/teletekst"); // configure traged server and url. 
  Serial.print          ("[HTTP] GET...nos.nl/teletekst\n");
  httpCode       =      http.GET();                               // start connection and send HTTP header.
  Serial.print          ("HTTPcode:");
  Serial.print          (httpCode); 
  error_action          (green,50,1);
  }
}
*/
void ConnectAtom(){
